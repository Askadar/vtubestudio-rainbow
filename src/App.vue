		...
		<n-layout-content content-style="padding: 24px">
			<n-space v-if="loading" justify="center">
				...
				<div v-if="failed">
					...
					<p v-if="typeof failed === 'string'">{{ failed }}</p>
				</div>
			</n-space>
			<n-space v-else vertical>
				<n-form :model="settings">
					<n-form-item label="Wildcard matcher which mesh to rainbow" path="meshMatch">
						<n-input
					...
					<n-form-item path="jebMode">
						<n-space vertical>
							<n-checkbox v-model:checked="settings.jebMode"
								>Whether to use simple rainbow provided by VTube Studio.</n-checkbox
...
import { firstValueFrom, ReplaySubject, interval, of } from 'rxjs'
import {
	take,
	mergeMap,
	catchError,
...
export default defineComponent({
	...
	async mounted() {
		...
		const $meshList = $ready.pipe(
			...
			tap((meshes) => (this.meshesList = meshes))
		)

		const $rainbow = this.enabled$.pipe(
			tap((enabled) => console.log({ enabled })),
			switchMap((enabled) =>
				enabled
		...
		firstValueFrom($rainbow.pipe(toArray(), take(3), toArray()))

		console.log(await firstValueFrom($meshList))
